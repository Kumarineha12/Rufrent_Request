...
const Photoshoot = () => {
  ...
  const [serviceDetails, setServiceDetails] = useState(null); // State to store service details from the database
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        ...
        if (isServiceFound) {
          setIsConfirmed(true);
          const tShirtDetails = data.services.find((service) => service.svc_id === 2);
          setServiceDetails(tShirtDetails);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Fetch the updated service details from the database
      const serviceResponse = await fetch(
        "http://localhost:5000/api/getServiceDetails?receipt_id=RRR-202503-004"
      );
      const serviceDataFromDB = await serviceResponse.json();

      // Find the photoshoot service details
      const photoshootDetails = serviceDataFromDB.services.find((service) => service.svc_id === 2);

      // Update the service status to "Requested" after claiming.
      setServiceStatus("Requested");
      setIsConfirmed(true);
      setServiceDetails(photoshootDetails); // Set the service details from the database
    } catch (error) {
...
const parsedSvcInfo = typeof svcInfo === "string" ? JSON.parse(svcInfo) : svcInfo; 
  ...
  return (
    <div className={`max-w-2xl mx-auto p-4 sm:p-6 ${tailwindStyles.whiteCard} rounded-lg shadow-lg relative`}>
      ...
      <div className="flex items-center justify-between px-3 py-4 rounded-t-lg flex-nowrap gap-1 sm:gap-3">
        ...
        <span
          className={`inline-flex items-center justify-center px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap shrink-0 ${serviceStatus === "Requested"
      ...
      {!isConfirmed ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className={`w-full py-2 rounded-md font-semibold transition duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isFormValid
        ...
        <div className="text-center">
          <div className="mb-6 bg-gray-50 p-4 rounded-md text-left max-h-64 overflow-y-auto">
            <p className="font-medium mb-2 text-md">
              {/* You've scheduled {serviceDetails?.svc_info ? JSON.parse(serviceDetails.svc_info).length : 0} appointment(s): */}
              You've scheduled {parsedSvcInfo ? Object.keys(parsedSvcInfo).length / 2 : 0} appointment(s):

            </p>
           
            <ul className="space-y-2 text-sm">
  {/* <li className="font-medium text-sm">Address: {parsedSvcInfo?.Address || ""}</li> */}
  {parsedSvcInfo && Object.entries(parsedSvcInfo).map(([key, value], index) => (
    key.includes("Date") && (
      <li key={index} className="border-b border-gray-100 pb-2 last:border-0">
        ...
        <span className="font-medium">{parsedSvcInfo[key]}</span> at{' '}
        <span>{parsedSvcInfo[key.replace("Date", "Time")]}</span>
      </li>
    )
  ))}
</ul>
