    ...
    const fetchServiceDetails = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/getServiceDetails?receipt_id=RRR-202503-002"
        );
        const data = response.data;

        const service = data.services.find((svc) => svc.svc_id === 1);
        if (service) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-4 rounded-t-lg">
        ...
        <span
          className={`px-3 py-1 rounded-full text-xs sm:text-sm font-medium flex-shrink-0 ${
      ...
      {serviceStatus === "Requested" ? (
        <div className="p-4 text-center">
          ...
          <p className={tailwindStyles.paragraph}>
            ...
            {serviceDetails?.svc_info && JSON.parse(serviceDetails.svc_info)["Slot Date"]}.
          </p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className={`w-full py-2 rounded-md font-semibold transition duration-300 focus:outline-none ${
              selectedDate
