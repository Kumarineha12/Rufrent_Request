...
import { fetchCities,fetchCommunities,fetchRms,fetchRecords,fetchRmFms } from '../services/newApiServices';
...
useEffect(() => {
 const fetchrm = async () => {
    ...
    try {
      const response = await fetchRms(selectedCommunity);
      setRm(response.data.result); 
      console.log(response.data.result,"rm selected")
    } catch (error) {
