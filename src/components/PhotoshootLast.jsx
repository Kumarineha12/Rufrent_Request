...
const Photoshoot = () => {
  ...
  const [address, setAddress] = useState(); // Static address initially set to "NULL"
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        ...
        // If you need to update additional service details, do it here.
        // For example:
        // if (isServiceFound) {
        //   setServiceDetails(data.services.find(service => service.svc_id === 3));
        // }
      } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg relative">
      ...
      <div className="flex items-center justify-between bg-white px-6 py-4 rounded-t-lg">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-xl font-semibold text-black mb-4">Photoshoot Appointment</h2>
        </div>
        {/* Always display the status badge */}
        <span
    ...
    {/* Address Section */}
     ...
     <div className="flex items-center bg-gray-50 ml-16 pl-9">
      <label className="text-md mr-16 font-semibold">Address  :</label>
      <input
        type="text"
        value={address}
        onChange={(e) => setAddress(e.target.value)}
        placeholder="Enter your address"
        className="text-sm text-center text-gray-900 bg-transparent border border-gray-300 rounded-md p-1 focus:outline-none focus:ring-2 focus:ring-blue-500 w-[70%]"
      />
    </div> 
